...
def main():
    ...
    global expression, AZIMUTH

    argv = sys.argv
    argc = len(argv)

    print('[uasge] python %s [<start azimuth(0)> <end azimuth(360)> <step azimuth(5)>]' % argv[0])
    print('equation is specified by \"equation.txt\"')
    ...
    # メインループ
    for AZIMUTH in np.arange(START, END, STEP):
